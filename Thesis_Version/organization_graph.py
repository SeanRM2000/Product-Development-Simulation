...
class OrganizationalGraph:
    ...
    def _aggregate_responsibilities_and_product_knowledge(self):
        ...
        product_knowledge = {}
        for element in sorted(list(all_architecture_elements)):
            product_knowledge[element] = 1 - self.architecture.architecture.nodes[element]['novelty']

        
        
        for member in self.all_agents:
            self.get_agent(member)['product_knowledge'] = {}
            self.get_agent(member)['product_information_consitency'] = {}
            self.get_agent(member)['product_information_completeness'] = {}
            experience = self.get_agent(member)['experience']
            
            for element in sorted(list(all_architecture_elements)):
                self.get_agent(member)['product_knowledge'][element] = product_knowledge[element] * experience
                self.get_agent(member)['product_information_consitency'][element] = {}
                self.get_agent(member)['product_information_completeness'][element] = {}
                for activity in self.get_agent(member)['responsibilities']:
                    self.get_agent(member)['product_information_consitency'][element][activity] = 0
                    self.get_agent(member)['product_information_completeness'][element][activity] = 0
                
            for element in sorted(list(all_architecture_elements)):
                for activity, product_elements in self.get_agent(member)['responsibilities'].items():
                    if activity in {'System_Design', 'Design'} and element in product_elements:
                        self.get_agent(member)['product_knowledge'][element] = 1   ################################# maybe change with the knowledge generated
                        self.get_agent(member)['product_information_consitency'][element][activity] = 1 # owner always has up-to-date information
                        self.architecture.architecture.nodes[element]['responsible_designer'] = member
                        self.get_agent(member)['responsible_element'] = element
...
if __name__ == "__main__":
    ...
    print(org_graph.get_agent('System Manager')['product_information_consitency'])
    
    print(org_graph.get_agent('System Manager')['expertise'])