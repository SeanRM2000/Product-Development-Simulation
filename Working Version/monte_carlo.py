...
class MonteCarlo():
    ...
    def run_montecarlo(self, print_additional_information=True, show_plots=True, create_samples=False):
        ...
        while sim_runs_left != 0:
            ...
            time_passed = time.time() - start_time
            sim_run_time = time.time() - sim_run_start_time
            print(f'\r[{(time_passed / 60):.1f} min ({sim_run_time:.1f} s)]: Run {sim_run}/{total_sim_runs} completed.', end='', flush=True)
        ...
        if self.use_seeds and create_samples:
            self.get_representative_samples(lead_times_in_weeks, costs_in_thousands, self.overall_utils, self.lead_time_target, self.cost_target)
...
def run_all_architecture_configurations(n_runs, use_seeds=True, configurations=None, create_samples=False, skip_errors=False):
    ...
    for config in configurations:
        ...
        sim.run_montecarlo(print_additional_information=False, show_plots=False, create_samples=create_samples)
...
if __name__ == "__main__":
    ...
    if False:
        ...
        if True:
            run_all_architecture_configurations(n_runs=1500, skip_errors=True, create_samples=True)
        else:
            sim = MonteCarlo(
                # Sim runs
                max_sim_runs = 200,
            ...
            sim.run_montecarlo(print_additional_information=False, create_samples=False)